# CircleAI
![CircleAI_readme](video/CircleAI_readme.png)
...
### 🔄 正則表達式轉換
- **AI 驅動生成**：利用 AI 智能分析文本模式並生成正則表達式
- **模式識別**：自動識別常見的數據格式（IP、Email、URL、日期等）
- **使用說明**：提供生成的正則表達式的詳細說明和使用範例
- **安全導向**：專為資訊安全分析場景優化的模式生成
- **多語言支援**：支援各種程式語言的正則表達式語法


### b64Decode 功能展示
![CircleAI_readme](video/b64_DecodeVideo.gif)
...
### 右鍵菜單功能總覽
CircleAI 提供三個主要的右鍵菜單功能，讓您能夠快速處理選定的文本：

#### 📋 動態菜單顯示
- **智能顯示**：右鍵菜單僅在選擇文本時顯示
- **文本預覽**：菜單項目會顯示選定文本的前 30 個字符
- **即時更新**：菜單標題會根據選定內容動態調整

### 🤖 AI 智能分析
1. 導航至任何網頁
2. 選擇您想要分析的文本
3. 右鍵點擊並從上下文選單中選擇「詢問 AI: [文本預覽...]」
4. 等待 AI 分析結果在彈出視窗中顯示
5. 分析結果將包含安全相關的見解和建議

**功能特色**：
- 專為資訊安全分析優化的 AI 提示
- 支援多種 AI 提供商（OpenAI、Gemini、Claude、Grok、自定義 API）
- 智能錯誤處理和重試機制

### 🔓 Base64 解碼功能
1. 導航至任何網頁
2. 選擇包含 Base64 字符串的文本
3. 右鍵點擊並選擇「解碼 Base64: [文本預覽...]」
4. 擴展將自動檢測並解碼 Base64 字符串
5. 如果解碼內容包含更多 Base64，將提供遞歸解碼選項
6. 使用「返回」按鈕在解碼歷史間導航

**功能特色**：
- 智能 Base64 檢測（最小 40 字符，嚴格格式驗證）
- 遞歸解碼支援（最多 5 層）
- 歷史導航功能
- 批量處理（單次最多 5 個字符串）
- UTF-8 和 UTF-16 編碼支援
...
1. 導航至任何網頁
2. 選擇您想要轉換為正則表達式的文本
3. 右鍵點擊並選擇「轉換為 RegEx: [文本預覽...]」
4. AI 將分析文本並生成相應的正則表達式
5. 結果將包含正則表達式模式和使用說明
